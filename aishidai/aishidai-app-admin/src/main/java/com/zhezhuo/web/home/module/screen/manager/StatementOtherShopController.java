...
@RequestMapping(value="/statemen/otherShop")
@Controller
public class StatementOtherShopController {
	...
	public String showStatemenList(
		...
		ShopDO otherShop = new ShopDO();
		//先判断登录的用户是否是店铺，如果不是就返回错误
		Result<ShopDO> otherShop_result = shopManager.queryShopDOBySysUserId(sYSUserId);
		
		otherShop = otherShop_result.getResult();
		if (otherShop == null) {
		...
		//商品消费总额
		double money_total_shop_otherShop = 0.0;
		...
		for (int i = 0; i < tradeOrderDO_list.size(); i++) {
			if (tradeOrderDO_list.get(i).getSellerUserId().longValue() 
				...
				money_total_shop_otherShop += Double.parseDouble(price);
			}
		}

		jsonObject.put("money_total_shop_otherShop", money_total_shop_otherShop);
		...
		for (int i = 0; i < servicesOrdersDO_list.size(); i++) {
			if (servicesOrdersDO_list.get(i).getSellerUserId().longValue() 
				...
				money_total_service_otherShop += Double.parseDouble(price);
			}
		}
		...
		money_total_otherShop = money_total_service_otherShop + money_total_shop_otherShop;
