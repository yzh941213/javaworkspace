...
function del(id){
	if(confirm("您确定要删除吗？")){
		$.post("delete.do",{autoids: id},function(data){
			if(data!="0"){
				alert("删除成功！");
				location.href="news.do?page=1&newsClsAutoid="+QueryString("newsClsAutoid")+"&title=";
			}
		});
	}
}
function search(){
	...
	location.href="news.do?page=1&newsClsAutoid=0&title="+title;
}
function showIndex(){
	var length=$("input[name=chk]:checked").size();
	if(length!=1){
		alert("请选中一项");
		return;
	}
	var id=$("input[name=chk]:checked").val();
	$.post("setShowIndex.do", {newsAutoid: id}, function(data){
		if(data!="0"){
			alert("置顶成功");
			location.reload();
		}
  ...
  <tr>
    <td height="30"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td background="${pageContext.request.contextPath}/themes/tab_05.gif"><span class="tablab"></span></td>
        <td background="${pageContext.request.contextPath}/themes/tab_05.gif">
        <table border="0" align="right" cellpadding="0" cellspacing="2">
            <tr>
              <td align="center"><a class="addbtn" href="addedit.do?newsClsAutoid=<%=request.getParameter("newsClsAutoid") %>&clsName=${clsName}">新增</a></td>              
              <td align="center"><a class="delbtn" href="javascript:delall();">删除</a></td>
              <td><a href="javascript:showIndex();">置顶</a></td>
              <td width="230" align="left"><input name="title" type="text" value="${title}"><a class="editbtn" href="javascript:search();">查询</a></td>
            </tr>
        </table></td>
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td bgcolor="#f3ffe3">
          ...
          <tr>
            <td height="26"><input name="chk" type="checkbox" value="${news.newsAutoid}" /></td>
            <td>${(pager.pageindex-1)*pager.pagesize+status.count}</td>
            <td>${news.title }</td>
            <td>${news.newsCls.name}</td>
            <td><fmt:formatDate value="${news.date}" pattern="yyyy-MM-dd"/></td>
            <td>${news.person.account}</td>
            <td>${news.sortIndex}</td>
            <td>${news.showIndex}</td>
            <td><a class="edittd" href="addedit.do?newsAutoid=${news.newsAutoid}&clsName=${news.newsCls.clsName}">编辑</a></td>
            <td><a class="deltd" href="javascript:del(${news.newsAutoid})">删除</a></td>
          </tr>
          </c:forEach>
