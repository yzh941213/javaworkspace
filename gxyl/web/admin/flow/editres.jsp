...
<script type="text/javascript" src="../../adminjs/flow/editres.js"></script>
<style>
      ...
      <tr>
        ...
        <td bgcolor="#f3ffe3">
          ...
          <tr>
            ...
            <td><input type="text" name="flowName" value="${flow.flowName}"/></td>
          </tr>
          ...
          <tr>
            <td height="26" align="right">资源文件：</td>
            <td><input type="button" value="添加文件" onclick="openUp('${flow.flowType}', 'file')"/>
            	<span id="spFile"><c:if test="${action eq 'modifyres.do'}">文件：【</c:if>${flow.fileName}<c:if test="${action eq 'modifyres.do'}">】；路径：【</c:if>${flow.filePath}<c:if test="${action eq 'modifyres.do'}">】</c:if></span>
            	<a id="dnexe" href="${pageContext.request.contextPath}${flow.filePath}" <c:if test="${action eq 'createres.do'}">style="display:none"</c:if>>下载</a>
          ...
          <tr>
          	...
          	<td><img id="img" width="240" height="180" src="${pageContext.request.contextPath}${flow.imageUrl}"/>
          		<a href="javascript:openUp('image', 'image');">上传图片</a>
