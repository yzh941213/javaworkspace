...
function del(id){
	if(confirm("您确定要删除吗？")){
		$.post("del.do", {friendAutoid:id}, function(data){
			if(data=="1"){
				alert("删除成功！");
				location.reload();
			}else{
  ...
  <tr>
    <td height="30"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td background="${pageContext.request.contextPath}/themes/tab_05.gif"><span class="tablab">相关链接</span></td>
      ...
      <tr>
        ...
        <td bgcolor="#f3ffe3">
         ...
         <c:forEach items="${list}" var="friend" varStatus="status">
          <tr>
            <td height="26"><input name="chk" type="checkbox" value="checkbox" /></td>
            <td>${(pager.pageindex-1)*pager.pagesize+status.count}</td>
            <td>${friend.name}</td>
            <td>${friend.url}</td>
            <td><a class="edittd" href="addedit.do?friendAutoid=${friend.friendAutoid}">编辑</a></td>
            <td><a class="deltd" href="javascript:del(${friend.friendAutoid})">删除</a></td>
          </tr>
         </c:forEach>
